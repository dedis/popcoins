...
const database = new couchbaseModule.Couchbase("database4");


database.createView("view4", "1", function(document, emitter) {
    emitter.emit(document._id, document);
});
function add(id, text){
    personList.splice(0);

    var rows = database.executeQuery("view4");
...
class User {
    ...
    setName(name, save) {
        ...
        if (save) {
            ...
            return add(FilesPath.USER_NAME, toWrite)
                .catch((error) => {
                console.log(error);
            console.dir(error);
            console.trace();
    ...
    setRoster(roster, save) {
        ...
        if (save) {
            ...
            return add(FilesPath.ROSTER, toWrite)
                .catch((error) => {
                console.log(error);
            console.dir(error);
            console.trace();
    ...
    load() {
        ...
        return Promise.all(promises)
            .then(
            this._isLoaded = true).catch(error => {
            console.log(error);
        console.dir(error);
        console.trace(error);
     ...
     */
    loadName() {
        var js = get(FilesPath.USER_NAME);
      try{  if(js!="null") {
         var jsonName =  js.text;
              let name;
     ...
     */
    loadRoster() {
        var js = get(FilesPath.ROSTER);
        var roster;
       try{ if(js!="null"){
          var  jsonRoster= js.text;

            if (jsonRoster.length > 0) {
             roster =  Convert.parseJsonRoster(jsonRoster);
        } else {
                 roster =EMPTY_ROSTER;
        }
