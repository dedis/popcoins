...
const OmniledgerRPC = require("./cothority/omniledger/OmniledgerRPC.js")
...
class User {
  ...
  /**
   * Removes all DARCs corresponding to this BaseID from the list of locally saved DARCs
   * @param {Uint8Array} baseID - the BaseID of the DARCs to be removed
   */
  removeDarc(baseID) {
  ...
  /**
   * Gets the list of locally saved DARCs
   * @returns {Promise} - a promise that yields a list of the DARCs once the DARCs are loaded and ready to be used
   */
  getDarcs() {
  ...
  /**
   * Saves local BC configuration into memory
   */
  saveBCConfig() {
    let toWrite = "";
  ...
  /*
   * Loads ByzCoin Config from memory
   */
  loadBCConfig() {
    return FileIO.getStringOf(FilePaths.BCCONFIG)
      ...
      .then(elems => {
        ...
        else return new Config(elems._byzcoinID, elems._roster);
      }).catch(err => Log.print(err));
  }

  /**
   * Gets the locally saved ByzCoin config
   * @returns {Promise} - a promise that yields the Config once its loaded and ready to be used
   */
  getBCConfig() {
  ...
  /**
   * Scans a ByzCoin configuration from a string exctracted from a QR code that has been generated by the bcadmin tool in Go
   * @param {string} str - String yielded by the QRCode generated from Go
   */
  scanBCConfig(str) {
    this.setBCConfig(new Config(str.match(/"ByzCoinID":"([^"]*)"/)[1]), this._roster)
    if (str.includes("Admin")) {
      this.getClient().then(cl => {
        this.addDarc(Darc.createDarcFromByzCoin(cl._genesisDarc))
      }).catch(err => Log.print(err.stack))

      let pubk = str.match(/Pub:ed25519:([^,]*)/)[1]
      let privk = str.match(/Priv:([^,]*)/)[1]

      Log.print("Public Key")
      Log.print(pubk)

      this._keyPair = new KeyPair(Convert.hexToByteArray(privk), Convert.hexToByteArray(pubk))
      return this._keyPair.save(FilePaths.KEY_PAIR)
    }
  }

  /**
   * Scans a DARC from a QR Code generated by another phone and saves it into memory
   * @param {string} str - String yielded by the QRCode generated from another PopCoins app
   */
  scanDarc(str) {
    const d = Darc.createDarcFromJSON(JSON.parse(str))
    this.addDarc(d)
    Log.print("Scanned DARC:")
    Log.print(d)
  }

  /**
   * Gets a client to communicate with ByzCoin from the Config registered for this User
   * @returns {Promise} - a promise that yields an OmniledgerRPC linked with the ByzCoin config registered for this User
   */
  getClient() {
    if (this._client == null)
      return OmniledgerRPC.fromKnownConfiguration(new Net.RosterSocket(this._roster, RequestPath.OMNILEDGER), Convert.base64ToByteArray(this._config._byzcoinID)).then(cl => {
        this._client = cl
        return this._client;
      }).catch(err => Log.print(err));
    else return Promise.resolve(this._client);
  }

  /**
   * Finds a DARC with which the User can spawn a new DARC
   * @returns {Darc} - The DARC to be used to spawn a new one ; if none is elligible, returns null
   */
  getGeneratingDarc() {
    for (var d of this._darcs) {
      Log.print(d)
      for (var [r, e] of d._rules) {
        if (r == "spawn:darc" && e.includes(this.getKeyPair().public.string())) {
          return d
        }
      }
    }
  ...
  /**
   * Checks whether the User has the right to evolve the DARC or not
   * If yes, evolves the DARC according to the specified action and returns it
   * If no, returns null
   * @param {Darc} darc - The DARC to be evolved
