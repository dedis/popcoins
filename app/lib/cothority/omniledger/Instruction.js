...
class Instruction {
    ...
    /**
     * Set the signers for this instruction
     *
     * @param signers {Identity[]} signers
     */
    set signers(signers){
        this._signers = signers.slice(0);
    }
    ...
    signBy(darcId, signers) {
        this._signers = signers.map(signer => {return signer.identity});
        const msg = this.toDarcRequest(darcId).hash();

        this._signatures = signers.map((signer) => {
